...
def run(staging_folder: str, s3_covd_dataset: str,
    ...
    if not os.path.exists(f"{staging_folder}/modification/"):
        os.makedirs(f"{staging_folder}/modification/")

    if not os.path.exists(f"{staging_folder}/modification/upload"):
        os.makedirs(f"{staging_folder}/modification/upload/")


    #  Pull down the skip_files_doc_parser.txt file
    print("Download Skip files doc parser file")
    Conf.s3_utils.download_file(
        file=f"{staging_folder}/modification/skip_file_doc_parser.txt",
        bucket="advana-raw-zone",
        object_path=f"gamechanger/projects/{gc_project}/data-pipelines/orchestration/repo/skip_file_doc_parser.txt",
    )


    # Download covid 19 dataset, currently being used
    print("Download Current covid 19 dataset")
    if Conf.s3_utils.object_exists(
        bucket="advana-raw-zone",
        object_path=f"gamechanger/projects/{gc_project}/raw_jsons/pdf_json.tar.gz",
    ):
        Conf.s3_utils.download_file(
            file=f"{staging_folder}/pdf_json.tar.gz",
            bucket="advana-raw-zone",
            object_path=f"gamechanger/projects/{gc_project}/raw_jsons/pdf_json.tar.gz",
        )

        with tarfile.open(f"{staging_folder}/pdf_json.tar.gz", "r:gz") as pdf_json_archive:
            pdf_json_archive.extractall(staging_folder)
    elif not os.path.exists(f"{staging_folder}/pdf_json/"):
        os.makedirs(f"{staging_folder}/pdf_json/")

    if Conf.s3_utils.object_exists(
        bucket="advana-raw-zone",
        object_path=f"gamechanger/projects/{gc_project}/raw_jsons/pmc_json.tar.gz",
    ):
        Conf.s3_utils.download_file(
            file=f"{staging_folder}/pmc_json.tar.gz",
            bucket="advana-raw-zone",
            object_path=f"gamechanger/projects/{gc_project}/raw_jsons/pmc_json.tar.gz",
        )

        with tarfile.open(f"{staging_folder}/pmc_json.tar.gz", "r:gz") as pmc_json_archive:
            pmc_json_archive.extractall(staging_folder)
    elif not os.path.exists(f"{staging_folder}/pmc_json/"):
        os.makedirs(f"{staging_folder}/pmc_json/")

    # Download the latest covid 19 dataset
    print("Download new COVID 19 dataset")
    print(f"gamechanger/projects/{gc_project}/cord_dataset/cord-19.tar.gz")
    Conf.s3_utils.download_file(
        file=f"{staging_folder}/cord-19.tar.gz",
        bucket="advana-raw-zone",
    ...
    extract_cord_19_raw_file(f"{staging_folder}/cord-19.tar.gz", staging_folder)

    # Create CORD 19 Diff Folder for PDF/PMC JSON file
    print("Create Diff between old covid and new covid dataset")
    create_diff(
        f"{staging_folder}/pmc_json/",
        f"{staging_folder}/document_parses/pmc_json/",
        staging_folder,
        "pmc_json",
    )

    create_diff(
        f"{staging_folder}/pdf_json/",
        f"{staging_folder}/document_parses/pdf_json/",
        staging_folder,
        "pdf_json",
    )
    ...
    run_pdf_generator(
        staging_folder,
        f"{staging_folder}/modification/pdf_json",
        f"{staging_folder}/metadata.csv",
        f"{staging_folder}/modification/pdf/",
        f"{staging_folder}/modification/skip_file_doc_parser.txt",
        multiprocess,
    )

    run_pdf_generator(
        staging_folder,
        f"{staging_folder}/modification/pmc_json",
        f"{staging_folder}/metadata.csv",
        f"{staging_folder}/modification/pdf",
        f"{staging_folder}/modification/skip_file_doc_parser.txt",
        multiprocess,
    ...
    run_docparser(
        staging_folder,
        f"{staging_folder}/modification/pdf",
        f"{staging_folder}/modification/json",
        multiprocess,
    )
    ...
    run_indexer(
        index_name=es_index,
        alias=es_alias,
        ingest_dir=f"{staging_folder}/modification/json",
    )
    ...
    Conf.s3_utils.upload_dir(
        local_dir=f"{staging_folder}/modification/pdf/",
        bucket="advana-raw-zone",
        prefix_path=f"gamechanger/projects/{gc_project}/pdf/",
    )
    ...
    Conf.s3_utils.upload_dir(
        local_dir=f"{staging_folder}/modification/json/",
        bucket="advana-raw-zone",
        prefix_path=f"gamechanger/projects/{gc_project}/json/",
    )
    ...
    delete_file_no_longer_need_in_s3(
        f"{staging_folder}/modification/files_to_remove.txt", gc_project
    )


    # Delete old records from Elasticsearch
    print("Delete file no longer need in S3 (PDF/Generated JSONs")
    delete_records_in_elasticsearch(
        file_list_records=f"{staging_folder}/modification/files_to_remove.txt",
        index_name=es_index,
        ingest_dir=f"{staging_folder}/modification/json",
    )
    ...
    make_tarfile(
        output_filename="pdf_json.tar.gz",
        work_cmd=f"{staging_folder}/document_parses/",
        source_dir="pdf_json",
    )

    make_tarfile(
        output_filename="pmc_json.tar.gz",
        work_cmd=f"{staging_folder}/document_parses/",
        source_dir="pmc_json",
    )
    ...
    Conf.s3_utils.upload_file(
        file=f"{staging_folder}/document_parses/pdf_json.tar.gz",
        bucket="advana-raw-zone",
        object_prefix=f"gamechanger/projects/{gc_project}/raw_jsons/",
    )

    Conf.s3_utils.upload_file(
        file=f"{staging_folder}/document_parses/pmc_json.tar.gz",
        bucket="advana-raw-zone",
        object_prefix=f"gamechanger/projects/{gc_project}/raw_jsons/",
    )
...
def extract_cord_19_raw_file(raw_cord_file: Union[str, Path], tmp_folder: Union[str, Path]):
    ...
    with tarfile.open(local_cord_dir_path, "r:gz") as archive:
        ...
        for file in os.listdir(f"{str(local_tmp_path.absolute())}/{os.path.commonprefix(archive.getnames())}"):
            if file == "document_parses.tar.gz":
                local_doc_files = Path(
                    (
                        f"{str(local_tmp_path.absolute())}/{os.path.commonprefix(archive.getnames())}"
                ...
                copyfile(str(local_metadata_path), f"{str(local_tmp_path)}/metadata.csv")
...
def create_diff(old_files: Union[str, Path], new_files: Union[str, Path], staging_folder: Union[str, Path],
    ...
    if not os.path.exists(f"{staging_folder}/modification/{folder_name}/"):
        os.makedirs(f"{staging_folder}/modification/{folder_name}/")
    ...
    for left_only in dc.left_only:
        # print(left_only)
        with open(f"{staging_folder}/modification/files_to_remove.txt", 'a') as remove_files:
    ...
    for right_only in dc.right_only:
        copyfile(
            f"{new_files}/{right_only}",
            f"{staging_folder}/modification/{folder_name}/{right_only}",
        )
    ...
    for diff in dc.diff_files:
        copyfile(
            f"{new_files}/{diff}",
            f"{staging_folder}/modification/{folder_name}/{diff}",
        )
...
def run_pdf_generator(staging_folder: Union[str, Path], input_raw_jsons_dir: Union[Path, str],
    ...
    if not os.path.exists(f"{str(staging_folder_path)}/modification/pdf/"):
        os.makedirs(f"{str(staging_folder_path)}/modification/pdf/")
...
def run_docparser(staging_folder: Union[str, Path], input_dir: Union[Path, str],
    ...
    if not os.path.exists(f"{str(staging_folder_path)}/modification/json/"):
        os.makedirs(f"{str(staging_folder_path)}/modification/json/")
...
def run_indexer(index_name: str, alias: str, ingest_dir):
    ...
    publisher = ConfiguredElasticsearchPublisher(
        index_name=index_name, ingest_dir=str(ingest_dir_path), alias=alias
    )
...
def delete_file_no_longer_need_in_s3(files_to_remove: Union[str, Path], gc_project: str):
    if not Path(files_to_remove).exists():
        return
    ...
    with open(files_to_remove_path, 'r') as remove_file:
        ...
        while line:
            ...
            Conf.s3_utils.delete_object(
                object_path=f"gamechanger/projects/{gc_project}/json/{line.strip()}",
                bucket="advana-raw-zone",
            )

            Conf.s3_utils.delete_object(
                object_path=f"gamechanger/projects/{gc_project}/pdf/{filename.strip()}.pdf",
                bucket="advana-raw-zone",
            )

            Conf.s3_utils.delete_object(
                object_path=f"gamechanger/projects/{gc_project}/pdf/{filename.strip()}.pdf.metadata",
                bucket="advana-raw-zone",
            )


            print(f"deleted {cnt}: {filename.strip()}")
            line = remove_file.readline()
            cnt += 1
...
def delete_records_in_elasticsearch(file_list_records: Union[str, Path], index_name: str, alias: str,  ingest_dir):
    ...
    if Path(file_list_records).exists():
        ...
        records = []
        with open(files_to_remove_path, 'r') as remove_file:
            while line := remove_file.readline():
                records.append(line)
