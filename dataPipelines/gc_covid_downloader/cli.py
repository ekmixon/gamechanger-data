...
def run(staging_folder: str) -> None:
    ...
    days_to_subtract = 1
    covd_download_date = datetime.now() - timedelta(days=days_to_subtract)
    d1 = covd_download_date.strftime("%Y-%m-%d")
    download_url = f"{covd_base_url}cord-19_{d1}.tar.gz"

    r = requests.get(download_url, allow_redirects=True)

    with open(f"{staging_folder}/covid_19.tar.gz", "wb") as covd_dataset:
        covd_dataset.write(r.content)
    filename = f"cord-19_{d1}.tar.gz"
    ...
    print(manifest_dic)
    with open(f"{staging_folder}/manifest.json", "w") as manifest_file:
