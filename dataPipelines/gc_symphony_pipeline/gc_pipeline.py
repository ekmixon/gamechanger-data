...
def run(staging_folder: str, skip_optional_ds: bool, aws_s3_json_prefix: str, aws_s3_pdf_prefix: str,
    ...
    Conf.s3_utils.download_dir(
        local_dir=f"{staging_folder}/pdf/", prefix_path=aws_s3_pdf_prefix
    )
    ...
    run_docparser(
        staging_folder,
        f"{staging_folder}/pdf/",
        f"{staging_folder}/json",
        multiprocess,
        skip_optional_ds,
    )

    shutil.make_archive(
        f'{staging_folder}/json_files' + date.today().strftime('%Y%m%d'),
        'zip',
        f"{staging_folder}/json/",
    )
...
def run_docparser(staging_folder: Union[str, Path], input_dir: Union[Path, str],
    ...
    if not os.path.exists(f"{str(staging_folder_path)}/json/"):
        os.makedirs(f"{str(staging_folder_path)}/json/")
